...
#if !(defined(USE_HIP) && USE_HIP) && !defined(LFS_MIN_SM)
...
namespace lfs::app {
    ...
    // CUDA binaries only carry code for SM >= LFS_MIN_SM, so a lower card can never JIT our
    ...
    bool preflightGpu(const bool show_dialog) {
        ...
        if (const cudaError_t err = cudaGetDeviceProperties(&prop, 0); err != cudaSuccess) {
            reportFatalStartupError(
                ...
                std::format("No usable ROCm GPU found ({}). Check that the AMD driver "
                            "and ROCm runtime support this GPU.",
                            cudaGetErrorString(err)),
                show_dialog);
            return false;
        }
        LOG_INFO("GPU: {} ({}, {} MB)", prop.name, prop.gcnArchName,
