...
namespace lfs::io {
    ...
    void PipelinedImageLoader::try_complete_pair(
        ...
        if (sidecar_ready_event) {
            cudaEvent_t& slot = depth ? pair.depth_ready_event : pair.normal_ready_event;
            if (slot) {
            ...
            try {
                ...
                if (auto cached = load_cached_jpeg_blob(result.cache_key)) {
                    ...
                    if (result.is_original_jpeg && !needs_requested_processing &&
                        ...
                        // The hot queue has no consumer without nvImageCodec;
                        // HIP and other CPU-decode builds use the cold workers.
