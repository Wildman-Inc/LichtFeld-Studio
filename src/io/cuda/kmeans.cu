...
#pragma unroll
            ...
            const auto group_mask = LFS_CUDA_SYNC_MASK;
...
#pragma unroll
                ...
                for (int delta = 2; delta > 0; delta /= 2) {
                    const float other = __shfl_xor_sync(LFS_CUDA_SYNC_MASK, best, delta, 4);
                    const int id = __shfl_xor_sync(LFS_CUDA_SYNC_MASK, best_id, delta, 4);
