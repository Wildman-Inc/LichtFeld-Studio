...
namespace lfs::rendering {
    namespace {
        ...
        externalMemoryGetMappedMipmappedArrayFunction() {
            ...
            // Some ROCm releases declare this API without exporting it from
            // libamdhip64.so. Treat it as an optional runtime capability.
    ...
    void setExpectedVulkanDeviceUuid(const std::array<std::uint8_t, 16>& uuid) {
        setExpectedVulkanDeviceIdentity(uuid);
    }
    ...
    bool cudaVulkanImageInteropSupported() {
        ...
        static const bool supported = [] {
            ...
            if (hipDeviceGetAttribute(
                    &image_supported, hipDeviceAttributeImageSupport, device) != cudaSuccess) {
                (void)cudaGetLastError();
                return false;
            }
    ...
    bool CudaVulkanInterop::copyTensorToSurface(const lfs::core::Tensor& tensor,
        ...
        if (!valid()) {
            return setFailure(last_error_, std::format(
                                               ...
                                               lfs::core::cuda_breadcrumb_arg(surface_),
