...
namespace fast_lfs::rasterization::kernels::forward {
    ...
    __global__ void preprocess_cu(
        ...
        if (lfs::core::cuda::ballot32(active) == 0)
            return;
    ...
    __device__ __noinline__ void create_instances_coop_warp(
        ...
        for (int src = 0; src < 32; ++src) {
            const bool src_active = __shfl_sync(LFS_CUDA_SYNC_MASK, static_cast<unsigned>(active), src, 32) != 0u;
            const uint src_n_scan = __shfl_sync(LFS_CUDA_SYNC_MASK, n_scan, src, 32);
            if (!src_active || src_n_scan == 0u)
                continue;
            const uint src_scan0 = __shfl_sync(LFS_CUDA_SYNC_MASK, scan0, src, 32);
            const uint src_cross0 = __shfl_sync(LFS_CUDA_SYNC_MASK, cross0, src, 32);
            const uint src_cross1 = __shfl_sync(LFS_CUDA_SYNC_MASK, cross1, src, 32);
            const uint src_write_end = __shfl_sync(LFS_CUDA_SYNC_MASK, write_offset_end, src, 32);
            const uint src_write = __shfl_sync(LFS_CUDA_SYNC_MASK, current_write_offset, src, 32);
            const uint src_prim = __shfl_sync(LFS_CUDA_SYNC_MASK, primitive_idx, src, 32);
            const uint src_dkey = __shfl_sync(LFS_CUDA_SYNC_MASK, depth_key, src, 32);
            const bool src_along_x = __shfl_sync(LFS_CUDA_SYNC_MASK, static_cast<unsigned>(scan_along_x), src, 32) != 0u;
            const float2 src_mean = make_float2(
                __shfl_sync(LFS_CUDA_SYNC_MASK, mean2d_shifted.x, src, 32),
                __shfl_sync(LFS_CUDA_SYNC_MASK, mean2d_shifted.y, src, 32));
            const float3 src_conic = make_float3(
                __shfl_sync(LFS_CUDA_SYNC_MASK, conic.x, src, 32),
                __shfl_sync(LFS_CUDA_SYNC_MASK, conic.y, src, 32),
                __shfl_sync(LFS_CUDA_SYNC_MASK, conic.z, src, 32));
            const float src_radius_sq = __shfl_sync(LFS_CUDA_SYNC_MASK, radius_sq, src, 32);
    ...
    __global__ void __launch_bounds__(config::block_size_blend_forward) blend_cu(
        ...
        for (int n_points_remaining = n_points_total, batch_base = 0;
            ...
            for (int j_base = 0; j_base < current_batch_size; j_base += 32) {
                ...
                unsigned mask0 = lfs::core::cuda::ballot32(intersects0);
                unsigned mask1 = lfs::core::cuda::ballot32(intersects1);
                if (warp_cull_mode == 1) {
