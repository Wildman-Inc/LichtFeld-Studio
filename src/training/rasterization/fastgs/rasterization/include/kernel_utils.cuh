    ...
    __device__ inline uint compute_exact_n_touched_tiles(
        ...
        for (int n = 0; n < n_remaining_threads && n < 32; n++) {
            ...
            const uint4 screen_bounds_coop = make_uint4(
                LFS_WARP_SHFL(screen_bounds.x, current_lane),
                LFS_WARP_SHFL(screen_bounds.y, current_lane),
                LFS_WARP_SHFL(screen_bounds.z, current_lane),
                LFS_WARP_SHFL(screen_bounds.w, current_lane));
            ...
            const float2 mean2d_shifted_coop = make_float2(
                LFS_WARP_SHFL(mean2d_shifted.x, current_lane),
                LFS_WARP_SHFL(mean2d_shifted.y, current_lane));
            const float3 conic_coop = make_float3(
                LFS_WARP_SHFL(conic.x, current_lane),
                LFS_WARP_SHFL(conic.y, current_lane),
                LFS_WARP_SHFL(conic.z, current_lane));
            const float power_threshold_coop = LFS_WARP_SHFL(power_threshold, current_lane);
            ...
            for (int i = 0; i < n_iterations; i++) {
                ...
                const uint contributes_ballot = LFS_WARP_BALLOT(contributes);
