...
#pragma unroll
        for (int offset = 16; offset > 0; offset /= 2) {
            local_max_contrib = ::max(local_max_contrib,
                                      __shfl_xor_sync(LFS_CUDA_SYNC_MASK, local_max_contrib, offset, 32));
        }
...
#pragma unroll
        ...
        for (int batch_base = 0; batch_base < T_eff; batch_base += batch_size) {
            ...
            for (int j_base = 0; j_base < n_batch; j_base += 32) {
                ...
                unsigned mask0 = lfs::core::cuda::ballot32(hit0);
                unsigned mask1 = lfs::core::cuda::ballot32(hit1);
                ...
                while (live != 0u) {
                    ...
                    const unsigned contrib_mask = lfs::core::cuda::ballot32(has_contribution);
                    if (contrib_mask != 0u && primitive_idx < n_primitives) {
                        ...
                        auto reduce_field = [&](float v) -> float {
                            if (n_contrib == 1u)
                                return __shfl_sync(LFS_CUDA_SYNC_MASK, v, src_lane, 32);
