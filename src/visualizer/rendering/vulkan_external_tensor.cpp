...
namespace lfs::vis {
    ...
    namespace {
        ...
        class InteropCleanup final {
        public:
            explicit InteropCleanup(Fn fn) : fn_(std::move(fn)) {}
            ~InteropCleanup() { fn_(); }
            InteropCleanup(const InteropCleanup&) = delete;
            InteropCleanup& operator=(const InteropCleanup&) = delete;
        private:
            Fn fn_;
        };
#endif
...
#if defined(_WIN32) && defined(USE_HIP) && USE_HIP
    VulkanCudaBufferMapping::~VulkanCudaBufferMapping() {
    ...
    std::expected<std::shared_ptr<VulkanCudaBufferMapping>, std::string>
    mapVulkanExternalBuffer(VulkanContext& context,
                            VulkanContext::ExternalBuffer& buffer,
                            const char* debug_name) {
        const auto handle = context.releaseExternalBufferNativeHandle(buffer);
        const InteropCleanup handle_cleanup{[handle] {
            if (VulkanContext::externalNativeHandleValid(handle)) {
                CloseHandle(handle);
            }
        }};
        if (!VulkanContext::externalNativeHandleValid(handle) ||
            buffer.size == 0 || buffer.allocation_size < buffer.size) {
            return std::unexpected("HIP buffer import requires a valid dedicated Vulkan buffer handle and allocation");
        }
        if (auto error = lfs::rendering::verifyCudaMatchesVulkanDevice(); error) {
            return std::unexpected(*error);
        }

        auto mapping = std::make_shared<VulkanCudaBufferMapping>();
        mapping->bytes = static_cast<std::size_t>(buffer.size);
        const auto failure = [debug_name](const char* operation, const cudaError_t status) {
            return std::unexpected(std::format(
                "Vulkan external buffer '{}' {} failed: {} ({})",
                debug_name ? debug_name : "<unnamed>", operation,
        ...
        auto status = cudaImportExternalMemory(&mapping->memory, &memory_desc);
        if (status != cudaSuccess) {
            return failure("cudaImportExternalMemory", status);
        }
        cudaExternalMemoryBufferDesc buffer_desc{};
        buffer_desc.size = buffer.size;
        status = cudaExternalMemoryGetMappedBuffer(&mapping->device_ptr, mapping->memory, &buffer_desc);
        if (status != cudaSuccess) {
            return failure("cudaExternalMemoryGetMappedBuffer", status);
        }
        if (mapping->device_ptr == nullptr) {
            return std::unexpected("HIP external buffer mapped to a null device pointer");
        }

        cudaStream_t stream = nullptr;
        const InteropCleanup stream_cleanup{[&stream] {
            if (stream != nullptr) {
        ...
        if (status != cudaSuccess) {
            return failure("cudaStreamCreateWithFlags", status);
        }
        status = cudaMemsetAsync(mapping->device_ptr, 0, static_cast<std::size_t>(buffer.size), stream);
        if (status != cudaSuccess) {
            return failure("cudaMemsetAsync", status);
        }
        status = cudaStreamSynchronize(stream);
        if (status != cudaSuccess) {
            return failure("cudaStreamSynchronize", status);
        }
        return mapping;
    }
    ...
    VulkanExternalTensorStorage::VulkanExternalTensorStorage(
        ...
        : VulkanExternalTensorStorage(context, std::move(buffer), bytes,
                                      std::move(debug_label), {},
                                      mapping ? mapping->device_ptr : nullptr) {
        cuda_mapping_ = std::move(mapping);
    }
#endif
...
#if defined(_WIN32) && defined(USE_HIP) && USE_HIP
        // HIP must drop its mapping/import before Vulkan frees the shared memory.
        cuda_mapping_.reset();
#endif
        if (context_) {
    ...
    std::expected<lfs::core::Tensor, std::string> makeVulkanExternalTensor(
...
#if defined(_WIN32) && defined(USE_HIP) && USE_HIP
        VulkanContext::ExternalBuffer exported{};
        const InteropCleanup buffer_cleanup{[&] {
            if (exported.buffer != VK_NULL_HANDLE) {
                context.destroyExternalBuffer(exported);
            }
        }};
        constexpr VkBufferUsageFlags usage =
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
            VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT;
        if (!context.createExternalBuffer(bytes, usage, exported,
        ...
        auto mapped = mapVulkanExternalBuffer(context, exported, debug_name);
        if (!mapped) {
            return std::unexpected(mapped.error());
        }
        void* const cuda_ptr = (*mapped)->device_ptr;
        auto owner = std::make_shared<VulkanExternalTensorStorage>(
            context, exported, bytes, debug_name ? debug_name : "<unnamed>",
            std::move(*mapped));
        exported = {};
        return lfs::core::Tensor::from_external_owner(
            cuda_ptr, std::move(shape), lfs::core::Device::CUDA, dtype,
            owner, cap_rows, nullptr, "vulkan_external_buffer");
#else
