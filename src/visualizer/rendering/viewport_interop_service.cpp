...
namespace lfs::vis {
    namespace {
        ...
        constexpr bool kUseLinearImageInterop = true;
#else
        constexpr bool kUseLinearImageInterop = false;
#endif
    ...
    struct ViewportInteropService::InteropPoolStorage {
        GpuResourcePool<PooledInteropUnit> pool{
            [](const PooledInteropUnit& unit) {
                return static_cast<std::size_t>(unit.image.allocation_size + unit.buffer.allocation_size);
            }};
    };
    ...
    void ViewportInteropService::drainInteropPool(VulkanContext& context, const bool force) {
        ...
        const auto destroy_fn = [&context](PooledInteropUnit& unit) {
            ...
            context.destroyExternalBuffer(unit.buffer);
    ...
    void ViewportInteropService::prepareChannel(VulkanContext& context,
        ...
        if (!kUseLinearImageInterop && !lfs::rendering::cudaVulkanImageInteropSupported()) {
            channel.disabled = true;
            resetChannel(channel);
            if (channel.policy.publishes_published) {
                clearPublished(channel);
            }
            LOG_WARN("{}: HIP device does not support Vulkan image interop; using the non-interop path",
                     channel.policy.failure_log_prefix);
            return;
        }
            ...
            if (auto hit = interop_pool_->pool.acquire(key)) {
                ...
                if (!context.createExternalImage(extent,
                                                 ...
                                                 std::format("{}.frame{}",
                                                             ...
                                                             frame_slot)) ||
                    (kUseLinearImageInterop &&
                     !context.createExternalBuffer(static_cast<VkDeviceSize>(extent.width) * extent.height * 4u,
                                                   VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                   created.buffer,
                                                   "vulkan.gui.interop_buffer",
                                                   std::format("{}.frame{}", channel.policy.debug_name_prefix, frame_slot))) ||
                    ...
                    if (created.image.image != VK_NULL_HANDLE || created.buffer.buffer != VK_NULL_HANDLE ||
                        ...
                        context.destroyExternalBuffer(created.buffer);
                ...
                const auto memory_handle = kUseLinearImageInterop
                                               ? context.releaseExternalBufferNativeHandle(created.buffer)
                                               : context.releaseExternalImageNativeHandle(created.image);
                ...
                lfs::rendering::CudaVulkanExternalImageImport image_import{
                    ...
                    .allocation_size = static_cast<std::size_t>(
                        kUseLinearImageInterop ? created.buffer.allocation_size : created.image.allocation_size),
                    .extent = {.width = extent.width, .height = extent.height},
                    .format = channel.policy.cuda_format,
                    .dedicated_allocation = kUseLinearImageInterop || context.externalMemoryDedicatedAllocationEnabled(),
                    .linear_buffer = kUseLinearImageInterop,
                };
    ...
    void ViewportInteropService::recordFrameBarriers(VkCommandBuffer frame_cb,
        ...
        for (const auto& pending : pending_frame_barriers_) {
            ...
            const VkImageLayout destination_layout = pending.unit->buffer.buffer != VK_NULL_HANDLE
                                                         ? VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL
                                                         : VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
            const auto destination = VulkanImageBarrierTracker::layoutAccess(
                destination_layout, VulkanImageBarrierTracker::AccessDirection::Destination);
        ...
        // Windows HIP writes linear exported memory; Vulkan performs the tiling
        // conversion. Source rows retain the allocation pitch while only the
        // tensor's valid top-left rectangle is copied.
        barriers.clear();
        for (const auto& pending : pending_frame_barriers_) {
            if (pending.unit == nullptr || pending.target == nullptr ||
                pending.unit->buffer.buffer == VK_NULL_HANDLE) {
                continue;
            }
            VkBufferImageCopy region{};
            region.bufferRowLength = static_cast<std::uint32_t>(pending.target->alloc_size.x);
            region.bufferImageHeight = static_cast<std::uint32_t>(pending.target->alloc_size.y);
            region.imageSubresource = {VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
            region.imageExtent = {static_cast<std::uint32_t>(pending.target->valid_size.x),
                                  static_cast<std::uint32_t>(pending.target->valid_size.y), 1};
            vkCmdCopyBufferToImage(frame_cb, pending.unit->buffer.buffer,
                                   pending.unit->image.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                   1, &region);
    ...
    void ViewportInteropService::shutdown(VulkanContext* context) {
        ...
        if (teardown_context_ && interop_pool_) {
            ...
            interop_pool_->pool.trimIdle([&](PooledInteropUnit& unit) {
                ...
                teardown_context_->destroyExternalBuffer(unit.buffer);
