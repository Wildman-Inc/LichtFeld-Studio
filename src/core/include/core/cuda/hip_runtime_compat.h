...
using cudaExternalMemory_t = hipExternalMemory_t;
using cudaExternalSemaphore_t = hipExternalSemaphore_t;
using cudaExternalMemoryHandleDesc = hipExternalMemoryHandleDesc;
using cudaExternalMemoryBufferDesc = hipExternalMemoryBufferDesc;
using cudaExternalMemoryMipmappedArrayDesc = hipExternalMemoryMipmappedArrayDesc;
using cudaExternalSemaphoreHandleDesc = hipExternalSemaphoreHandleDesc;
using cudaExternalSemaphoreWaitParams = hipExternalSemaphoreWaitParams;
using cudaExternalSemaphoreSignalParams = hipExternalSemaphoreSignalParams;
using cudaExternalMemoryHandleType = hipExternalMemoryHandleType;
using cudaExternalSemaphoreHandleType = hipExternalSemaphoreHandleType;
...
#ifndef cudaChannelFormatKindUnsigned
...
#ifndef cudaArraySurfaceLoadStore
...
#ifndef cudaExternalMemoryHandleTypeOpaqueFd
...
#ifndef cudaExternalSemaphoreHandleTypeOpaqueFd
...
#ifndef cudaExternalMemoryDedicated
...
#ifndef cudaImportExternalMemory
...
#define cudaDevAttrTimelineSemaphoreInteropSupported 1000001
#endif

namespace lfs::cuda_compat {
    inline hipError_t device_get_attribute(int* value, int attr, int device) {
        if (attr == cudaDevAttrTimelineSemaphoreInteropSupported) {
            hipDeviceProp_t props{};
            const hipError_t status = hipGetDeviceProperties(&props, device);
            if (status != hipSuccess) {
                return status;
            }
            *value = props.timelineSemaphoreInteropSupported;
            return hipSuccess;
        }
        return hipDeviceGetAttribute(value, static_cast<hipDeviceAttribute_t>(attr), device);
    }
} // namespace lfs::cuda_compat

#define cudaDeviceGetAttribute ::lfs::cuda_compat::device_get_attribute
...
#endif
#ifndef CU_MEM_HANDLE_TYPE_WIN32
#define CU_MEM_HANDLE_TYPE_WIN32 hipMemHandleTypeWin32
#endif
#ifndef CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR
#define CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR hipMemHandleTypePosixFileDescriptor
#endif
#ifndef CU_DEVICE_ATTRIBUTE_VIRTUAL_ADDRESS_MANAGEMENT_SUPPORTED
#define CU_DEVICE_ATTRIBUTE_VIRTUAL_ADDRESS_MANAGEMENT_SUPPORTED hipDeviceAttributeVirtualMemoryManagementSupported
#endif
#ifndef CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_WIN32_HANDLE_SUPPORTED
#define CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_WIN32_HANDLE_SUPPORTED hipDeviceAttributeMemoryPoolSupportedHandleTypes
#endif
#ifndef CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR_SUPPORTED
#define CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR_SUPPORTED hipDeviceAttributeMemoryPoolSupportedHandleTypes
#endif

inline CUresult lfsCuGetErrorName(CUresult r, const char** name) {
    if (name) {
        *name = hipGetErrorName(r);
    }
    return CUDA_SUCCESS;
}

inline CUresult lfsCuGetErrorString(CUresult r, const char** desc) {
    if (desc) {
        *desc = hipGetErrorString(r);
    }
    return CUDA_SUCCESS;
}
...
#define cuGetErrorName lfsCuGetErrorName
#define cuGetErrorString lfsCuGetErrorString
...
#if LFS_USE_HIP
#define LFS_CUDA_SYNC_MASK 0xffffffffffffffffull
#else
