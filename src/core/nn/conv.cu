...
#pragma unroll
    ...
    void conv2d_implicit(const void* input, const void* weight, const void* weight_taps,
        ...
        // The WMMA and MMA implementations contain CUDA-specific instructions.
        // HIP uses the shared-memory SIMT kernel regardless of CUDA path overrides.
        if (LFS_USE_HIP || force_simt) {
            launch_simt();
            return;
        }
