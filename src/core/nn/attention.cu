...
#pragma unroll
            for (int offset = 16; offset > 0; offset >>= 1) {
                v += __shfl_xor_sync(LFS_CUDA_SYNC_MASK, v, offset, 32);
            }
            return v;
        }
...
#pragma unroll
            for (int offset = 16; offset > 0; offset >>= 1) {
                const float other = __shfl_xor_sync(LFS_CUDA_SYNC_MASK, v, offset, 32);
                v = fmaxf(v, other);
            }
            return v;
        }
...
#pragma unroll
                    for (int r = 0; r < 2; ++r) {
                        row_max[r] = fmaxf(row_max[r], __shfl_xor_sync(LFS_CUDA_SYNC_MASK, row_max[r], 1, 32));
                        row_max[r] = fmaxf(row_max[r], __shfl_xor_sync(LFS_CUDA_SYNC_MASK, row_max[r], 2, 32));
                    }
...
#pragma unroll
                    for (int r = 0; r < 2; ++r) {
                        l_add[r] += __shfl_xor_sync(LFS_CUDA_SYNC_MASK, l_add[r], 1, 32);
                        l_add[r] += __shfl_xor_sync(LFS_CUDA_SYNC_MASK, l_add[r], 2, 32);
