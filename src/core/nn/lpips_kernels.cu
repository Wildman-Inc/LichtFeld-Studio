        ...
        __global__ void __launch_bounds__(kReduceThreads)
            ...
            for (int c = c_begin; c < c_end; ++c) {
                ...
                if constexpr (kPool) {
                    ...
                    mx = fmaxf(mx, __shfl_xor_sync(LFS_CUDA_SYNC_MASK, mx, 1, 32));
                    my = fmaxf(my, __shfl_xor_sync(LFS_CUDA_SYNC_MASK, my, 1, 32));
                    if (pool_ok) {
...
#pragma unroll
            for (int offset = 16; offset > 0; offset >>= 1) {
                score += __shfl_xor_sync(LFS_CUDA_SYNC_MASK, score, offset, 32);
            }
            if (lane == 0) {
