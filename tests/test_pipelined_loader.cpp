...
TEST_F(PipelinedImageLoaderTest, OriginalJpegUsesAvailableDecoderWithoutReencoding) {
    const bool gpu_decode = NvCodecImageLoader::is_available();
    for (const bool high_precision : {false, true}) {
        ...
        ASSERT_TRUE(ready) << "Original JPEG was queued without an available decoder";
        ASSERT_TRUE(ready->tensor.is_valid());
        ...
        input.sequence_id = 1;
        loader.prefetch({input});
        const auto repeated = loader.try_get_for(std::chrono::seconds{10});
